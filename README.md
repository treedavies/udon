...
* A research project built on 20 minutes a day.
...
DNS is required for clients to communicate with the server's FQDN.
...
Run the `--init` flag as a regular user.
...
Sally's config for the channel
filename: `bob_and_sally`

    channel = "bob_and_sally"
    client_key_name = 'sally.pub'
...
This file is generated by `udon --init`.
...
# 7.1 server.conf with Certbot (LetsEncrypt)

To configure the server when your certificate is provided by LetsEncrypt, you can 
find the `ssl_cert` and `ssl_cert_key` at the following paths.

  /etc/letsencrypt/live/<DOMAIN>/cert.pm
  /etc/letsencrypt/live/<DOMAIN>/privkey.pem

Clients should use the `chain.pem` located at:
